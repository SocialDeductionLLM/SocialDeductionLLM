...
full_name = f"Crewmate_{args.num_imposters + args.num_crewmates}_{RWKV_NAME.split('-')[-1]}_{args.other_seed}_{args.seed}_{args.name}"
...
for iteration in tqdm(range(args.num_iterations), desc=" outer", position=0):
    ...
    for _ in range(args.re_iterations):
        past_accuracies = []
        for epoch in range(args.update_epochs):
            cur_losses = utils.do_eval(args, cbuf, c_model, advantages_mean, advantages_std, epoch, True, args.update_epochs, c_optimizer, ho_model=o_model)
            if len(cur_losses) != 0:
