...
full_name = f"Crewmate_{args.num_imposters + args.num_crewmates}_{RWKV_NAME.split('-')[-1]}_{args.seed}_{args.name}"
...
hybrid_c_model = buffer_generator_full.HybridModel2(buffer_generator_full.DummyModel(is_imposter=False), c_model)
...
for iteration in tqdm(range(args.num_iterations), desc=" outer", position=0):
    ...
    for _ in range(args.re_iterations):
        for epoch in range(args.update_epochs):
            ...
            cur_losses = utils.do_train(args, cbuf, c_model, advantages_mean, advantages_std, epoch, True, args.update_epochs, c_optimizer, ho_model=o_model)
            for l in cur_losses:
                all_losses[l] += cur_losses[l]
            all_losses['minipile'] += utils.do_pile_training(args, c_model, dataset, tokenizer, c_optimizer)
