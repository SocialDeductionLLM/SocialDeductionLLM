...
def load_wkv_cuda_kernel(context_length):
    ...
    rwkv_cuda_kernel = load_kernel(
        name=f"rwkv_{context_length}",
        sources=cuda_kernel_files,
        verbose=(logging.get_verbosity() == logging.DEBUG),
        extra_cuda_cflags=flags,
        extra_cflags=["-std=c++17", "-O3", f"-DTmax={context_length}"]
    )
...
class RwkvLinearAttention(torch.autograd.Function):
    @staticmethod
    def forward(ctx, time_decay, time_first, key, value, state=None, return_state=False, lengths=None):
        ...
        if return_state or state is not None:
            ...
            forward_func(time_decay, time_first, key, value, output, state, lengths)
        else:
    ...
    def backward(ctx, g_output, g_state=None):
        #print("BEFORE STARTING BACKWARD")
        ...
        #print("output", output)
        #time.sleep(0.1)
        #print(time_decay.shape, g_time_decay.shape, time_first.shape, g_time_first.shape)
        backward_func(
            time_decay,
            time_first,
            key,
        ...
        #time.sleep(0.1)
        #print("gtf", g_time_first)
        #print("gkey", g_key)
        #print("gvalue", g_value)
        #print("gtd", g_time_decay)
        #print(g_time_decay, g_time_first, g_key, g_value)
        #print("DONE BACKWARD")
...
def rwkv_linear_attention_cpu(time_decay, time_first, key, value, state=None, return_state=False, lengths=None):
    ...
    if lengths is None:
        lengths = torch.ones(key.size(0), dtype=torch.long, device=key.device) * seq_length
    ...
    for current_index in range(seq_length):
        ...
        num_state[lengths > current_index] = (e1 * num_state + e2 * current_value)[lengths > current_index]
        den_state[lengths > current_index] = (e1 * den_state + e2)[lengths > current_index]
        max_state[lengths > current_index] = max_for_state[lengths > current_index]
...
def rwkv_linear_attention(time_decay, time_first, key, value, state=None, return_state=False, lengths=None):
    no_cuda = any(t.device.type != "cuda" for t in [time_decay, time_first, key, value])
    # Launching the CUDA kernel for just one token will actually be slower (there is no for loop in the CPU version
    # in this case).
    one_token = key.size(1) == 1
    #print(one_token, lengths, state)
    if (rwkv_cuda_kernel is None or no_cuda or one_token) and (lengths is not None):
        return rwkv_linear_attention_cpu(time_decay, time_first, key, value, state=state, return_state=return_state, lengths=lengths)
    else:
        if lengths is None:
            #print("NO LENGTHS")
            #print(type(state))
            #print(state.shape)
            #print(state.shape)
            state = torch.movedim(state, 0, 2)
            #print(type(state))
            #print(state.shape)
            out, new_state = torch.ops.rwkv.wkv(time_decay, time_first, key, value, state)
            return out, torch.movedim(new_state, 2, 0)
            #lengths = torch.ones(key.size(0), dtype=torch.long, device=key.device) * key.size(1)
        return RwkvLinearAttention.apply(time_decay, time_first, key, value, state, return_state, lengths)
...
class RwkvSelfAttention(nn.Module):
    ...
    def extract_key_value(self, hidden, state=None, lengths=None):
        # Mix hidden with the previous timestep to produce key, value, receptance
        #if hidden.size(1) == 1 and state is not None:
        #    shifted = state[1][:, :, self.layer_id].to(dtype=hidden.dtype)
        if True:#else:
        ...
        if state is not None:
            if lengths is None:
                ...
                state[1][:, :, self.layer_id] = hidden[torch.arange(hidden.size(0)), lengths - 1]
        return receptance, key, value, state

    def forward(self, hidden, state=None, use_cache=False, lengths=None):
        receptance, key, value, state = self.extract_key_value(hidden, state=state, lengths=lengths)
        if isinstance(state, torch.Tensor):
            layer_state = state[2:, :, :, self.layer_id]
        else:
...
class RwkvFeedForward(nn.Module):
    ...
    def forward(self, hidden, state=None, lengths=None):
        #if hidden.size(1) == 1 and state is not None:
        #    shifted = state[0][:, :, self.layer_id].to(dtype=hidden.dtype)
        #else:
        if True:
            shifted = self.time_shift(hidden)
            if state is not None:
        ...
        if state is not None:
            if lengths is None:
                ...
                state[0][:, :, self.layer_id] = hidden[torch.arange(hidden.size(0)), lengths - 1]
...
class RwkvBlock(nn.Module):
    ...
    def forward(self, hidden, state=None, use_cache=False, output_attentions=False, lengths=None):
        if self.layer_id == 0:
            hidden = self.pre_ln(hidden)

        attention, state = self.attention(self.ln1(hidden), state=state, use_cache=use_cache, lengths=lengths)
        hidden = hidden + attention
        
        feed_forward, state = self.feed_forward(self.ln2(hidden), state=state, lengths=lengths)
    ...
    def forward(
        ...
        if use_cache and state is None:
            if lengths is None:
                ...
                state = torch.stack([
                    torch.zeros(
                        *shape, dtype=torch.float32, device=inputs_embeds.device
                    )
                    for i in range(5)
                ], dim=0)
                state[4] -= 1e30
        ...
        for idx, block in enumerate(self.blocks):
            if self.gradient_checkpointing and self.training:
                hidden_states, state, attentions = self._gradient_checkpointing_func(
                    block.__call__, hidden_states, state, use_cache, output_attentions, lengths
                )
            else:
                hidden_states, state, attentions = block(
                    hidden_states, state=state, use_cache=use_cache, output_attentions=output_attentions, lengths=lengths
                )
...
class RwkvForCausalLM(RwkvPreTrainedModel):
    ...
    def forward(
        ...
        """
        self.collected_statistics.append(input_ids.shape)
        #return RwkvCausalLMOutput(loss=None, logits=torch.ones((input_ids.shape[0], input_ids.shape[1], 65536), device='cuda'), state=state, hidden_states=None, attentions=None)
...
class RwkvForCausalLMAction(RwkvPreTrainedModel):
    _tied_weights_keys = ["head.weight"]
    ...
    def __init__(self, config):
        ...
        self.collected_statistics = []
        self.rwkv = RwkvModel(config)
        self.middle_head = nn.Linear(config.hidden_size, config.hidden_size, bias=False)
        self.head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
    ...
    def forward(
        ...
        hidden_states = rwkv_outputs[0]
        # print(torch.max(hidden_states, dim=-1), torch.min(hidden_states, dim=-1))

        middle_logits = self.middle_head(hidden_states)
        logits = self.head(nn.functional.leaky_relu(middle_logits+10, negative_slope=0.1)-10)
...
def generate_action_model(original):
    answer = RwkvForCausalLMAction(original.config).to(device=original.device)
    answer.rwkv = original.rwkv
    answer.head = original.head
    answer.middle_head.to(dtype=original.head.weight.dtype)
    nn.init.xavier_uniform_(answer.middle_head.weight, gain=0.01)
    with torch.no_grad():
        answer.middle_head.weight += torch.eye(original.config.hidden_size, device=original.device)
    return answer
