...
//     const int B, const int T, const int C, const F *__restrict__ const _w, const F *__restrict__ const _u,
//     const F *__restrict__ const _k, const F *__restrict__ const _v, F *__restrict__ const _y, F *__restrict__ const _s, long *__restrict__ const _lengths
// ) {
//     const int idx = blockIdx.x * blockDim.x + threadIdx.x;
//     const int _b = idx / C;
//     const int _c = idx % C;
...
//     assert(B * C % threadsPerBlock.x == 0);
//     dim3 numBlocks(B * C / threadsPerBlock.x);
//     kernel_forward_with_state<<<numBlocks, threadsPerBlock>>>(B, T, C, w, u, k, v, y, s, lengths);
// }
